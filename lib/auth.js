...
export async function verifyStaff(req, section_id = null) {
  try {
    ...
    const isSuperAdmin = isSuperAdminRaw === 1;
    ...
    const targetKey = String(section_id);
    const sectionData = roles[targetKey];

    if (!sectionData && !isSuperAdmin) {
      return { 
        error: NextResponse.json({ message: "Forbidden: no access to this section" }, { status: 403 }) 
      };
    }

    let roleChar, counter_id = null;

    if (Array.isArray(sectionData)) {
      [roleChar, counter_id] = sectionData;
    } else {
      roleChar = sectionData;
    }

    const roleName = roleChar === 'a' ? 'admin' : 'staff';
