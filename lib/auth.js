...
export async function verifyStaff(req, section_id = null) {
  try {
    ...
    const counter_id = rows[0].counter_id
    ...
    if (section_id !== null) {
      ...
      const roleInSection = roles.find(r => r.section_id === section_id);

      if (!roleInSection && !isSuperAdmin) {
        return {
          error: NextResponse.json(
      ...
      isAdmin = roles.some(
        r => r.role === "admin" && r.section_id === section_id
      );
    }
