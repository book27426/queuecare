...
export async function verifyStaff(req, section_id = null) {
  try {
    ...
    const { staff_id, isSuperAdmin, roles = [] } = decoded;
    ...
    const sectionRole = roles.find(r => r.section_id === targetSection);

    if (!sectionRole && !isSuperAdmin) {
      return { 
        error: NextResponse.json({ message: "Forbidden: no access" }, { status: 403 }) 
      };
    }
